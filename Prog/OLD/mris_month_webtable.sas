** program name:mris_month_webtable.sas
** previous program name: none
** following program name:
** project name: HNC
** Description:  This program prepares the MONTHLY MRIS data for the metro area by subarea and county
to output to web tables**
** Date Created: 4-25-09
** Updated: 1-22-2010
** ksp
***********************************************;
*lhendey libnames;
...
proc sort data=hnc.mriscnty&year.m&month.;
by uiorder;

proc summary data=hnc.mriscnty&year.m&month.;
id ui_div;
class uiorder;
...
data newyear&year.m&month (drop=_type_);
 	merge hnc.mriscnty&year.m&month. (drop = pdays120p_&year.m&month. pdays30_&year.m&month. pdays60_&year.m&month. 
 ...
 ***sales by category***;
 ...
 set hnc.mriscat30_500_cnty&year.m&month (where = (pricecat = "Totals")
...
%if ucounty ~= '00000' %then %do;
	...
	%do c = 1 %to 3;
	 ...
	 %*dollar_convert_month( &clist. , &clistR., &mwd&year., _Jun2009, quiet=N, mprint=y, series=CUUR0000SA0L2 );
	%end;
%end;
...
data hnc.mris199701_200912(drop  =_type_);
	set divfile
		mris199701_200912;
...
proc sort data=hnc.mris199701_200912;
	by uiorder ucounty;
	run;
